...
export default function Words() {
    ...
    return (
        <section className='flex flex-col justify-between max-sm:flex-col bg-black px-[80px] max-md:px-[50px] max-sm:px-[30px] py-[80px] max-md:py-[50px] max-sm:py-[40px] max-sm:gap-[40px] gap-[48px]'>
            ...
            <div className='grid grid-cols-3 gap-[30px] max-md:grid-cols-2 max-sm:grid-cols-1'>
                {data.map((item, index) => {
                    return (
                        <button key={index} onClick={() => handleNavigate(item.path)} className='rounded-[20px] flex flex-col max-sm:flex-row items-center bg-grey px-5 pt-[10px] pb-[30px] cursor-pointer'>
                            <img src={item.image} alt="Wallet" className='max-w-[250px] max-g-[250px] max-md:max-w-[160px] max-md:max-h-[160px] max-sm:max-w-[100px] max-sm:max-h-[100px]' />
                            <div className='flex flex-col gap-[10px]'>
